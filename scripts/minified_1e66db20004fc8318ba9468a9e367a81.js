...
$(item).css('display','block');$(item).css('top',$(this).offset().top+$(this).height());$(item).css('left',Math.max($(this).offset().left-$(item).width()+$(this).outerWidth(),0));$(item).height($(item).find('div:first').height())});$(document).click(function(){$(item).css('display','none')})});$(document).on('click','.you_sure',function(){var custom_message=$(this).attr('data-confirm');return confirm(custom_message?custom_message.replace(/-n-/g,"\n"):smf_you_sure)});$('.smf_select_text').on('click',function(e){e.preventDefault();var actOnElement=$(this).attr('data-actonelement');return typeof actOnElement!=="undefined"?smfSelectText(actOnElement,!0):smfSelectText(this)})});function avatar_fallback(e){var e=window.e||e;var default_avatar='/avatars/default.png';var default_url=document.URL.substr(0,smf_scripturl.lastIndexOf('/'))+default_avatar;if(e.target.tagName!=='IMG'||!e.target.classList.contains('avatar')||e.target.src===default_url)
return;e.target.src=default_url;return!0}
if(document.addEventListener)
document.addEventListener("error",avatar_fallback,!0);else document.attachEvent("error",avatar_fallback);$(function(){$('ul.dropmenu, ul.quickbuttons').superfish({delay:250,speed:100,sensitivity:8,interval:50,timeout:1});$('.preview').SMFtooltip();$('a.bbc_link img.bbc_img').parent().css('border','0')});function smf_codeBoxFix()
...
base.sourceEditorInsertText(' '+$(this).attr('alt')+' ');else base.wysiwygEditorInsertHtml(start+'<img src="'+$(this).attr("src")+'" data-sceditor-emoticon="'+$(this).attr('alt')+'">'+end);e.preventDefault()}))},createPermanentDropDown:function(){var emoticons=$.extend({},this.opts.emoticons.dropdown);var popup_exists=!1;content=$('<div class="sceditor-insertemoticon">');line=$('<div>');base=this;for(smiley_popup in this.opts.emoticons.popup)
...
attribs+=' title="'+attrs.title+'"';return '<img'+attribs+' src="'+content+'">'}});sceditor.formats.bbcode.set('attach',{tags:{attach:{src:null}},allowsEmpty:!0,quoteType:$.sceditor.BBCodeParser.QuoteType.never,format:function(element,content){var element=$(element),attribs='',style=function(name){return element.style?element.style[name]:null};if(element.attr('width')||style('width'))
attribs+=" width="+$(element).width();if(element.attr('height')||style('height'))
attribs+=" height="+$(element).height();if(element.attr('alt'))
attribs+=" alt="+element.attr('alt');if(element.attr('title'))
attribs+=" name="+element.attr('title');if(element.attr('data-type'))
attribs+=" type="+element.attr('data-type');return '[attach'+attribs+']'+(element.attr('data-attachment')?element.attr('data-attachment'):content)+'[/attach]'},html:function(token,attrs,id){var parts,attribs='';if(!$.isNumeric(id)||Math.floor(id)!=+id||+id<=0){if(typeof attrs.width!=="undefined")
attribs+=' width='+attrs.width;if(typeof attrs.height!=="undefined")
attribs+=' height='+attrs.height;if(typeof attrs.alt!=="undefined")
attribs+=' alt='+attrs.alt;if(typeof attrs.name!=="undefined")
attribs+=' name='+attrs.name;if(typeof attrs.type!=="undefined")
attribs+=' type='+attrs.type;return '[attach'+attribs+']'+id+'[/attach]'}
attribs+=' data-attachment="'+id+'"'
if(typeof attrs.width!=="undefined")
attribs+=' width="'+attrs.width+'"';if(typeof attrs.height!=="undefined")
attribs+=' height="'+attrs.height+'"';if(typeof attrs.alt!=="undefined")
attribs+=' alt="'+attrs.alt+'"';if(typeof attrs.type!=="undefined")
attribs+=' data-type="'+attrs.type+'"';if(typeof attrs.name!=="undefined")
attribs+=' title="'+attrs.name+'"';if((typeof attrs.type!=="undefined"&&attrs.type.indexOf("image")===0)){var contentUrl=smf_scripturl+'?action=dlattach;attach='+id+';type=preview;thumb';contentIMG=new Image();contentIMG.src=contentUrl}
if(typeof contentUrl==="undefined"||contentIMG.getAttribute('width')==0)
return '<a href="'+smf_scripturl+'?action=dlattach;attach='+id+';type=preview;file"'+attribs+'>'+(typeof attrs.name!=="undefined"?attrs.name:id)+'</a>';else return '<img'+attribs+' src="'+contentUrl+'">'}});sceditor.formats.bbcode.set('url',{allowsEmpty:!0,quoteType:$.sceditor.BBCodeParser.QuoteType.never,tags:{a:{href:null}},format:function(element,content){var element=$(element),url=element.attr('href');if(url.substr(0,7)==='mailto:')
...
{var attribs='';if(typeof element.attr('title')!=="undefined")
attribs+=' name='+element.attr('title');if(typeof element.attr('data-type')!=="undefined")
attribs+=' type='+element.attr("data-type");return '[attach'+attribs+']'+element.attr('data-attachment')+'[/attach]'}
...
return content;floatclass=attrs.defaultattr.indexOf('left')==0?'floatleft':'floatright';style=typeof attrs.max!=="undefined"?' style="max-width:'+attrs.max+(+attrs.max===parseInt(attrs.max)?'px':'')+';"':'';return '<div class="'+floatclass+'"'+style+'>'+content+'</div>'}});sceditor.formats.bbcode.set('youtube',{allowsEmpty:!0,tags:{div:{class:'videocontainer'}},format:function(element,content){youtube_id=$(element).find('iframe').data('youtube-id');if(typeof youtube_id!=="undefined")
return '[youtube]'+youtube_id+'[/youtube]';else return content},html:'<div class="videocontainer"><div><iframe frameborder="0" src="https://www.youtube.com/embed/{0}?wmode=opaque" data-youtube-id="{0}" allowfullscreen></iframe></div></div>'});(function(document,sceditor){'use strict';var dom=sceditor.dom;var ytUrlRegex=/(^|\s)(?:https?:\/\/)?(?:www\.)?(?:youtu\.be\/|youtube\.com\/watch\?v=)([^"&?\/ ]{11})(?:\&[\&_\?0-9a-z\#]+)?(\s|$)/i;function youtubeEmbedCode(id){return '<div class="videocontainer"><div><iframe frameborder="0" '+'src="https://www.youtube-nocookie.com/embed/'+id+'" '+'data-youtube-id="'+id+'" allowfullscreen></iframe></div></div>'}
function convertYoutubeLinks(root){var node=root.firstChild;while(node){if(node.nodeType===3){var text=node.nodeValue;var parent=node.parentNode;var match=text.match(ytUrlRegex);if(match){parent.insertBefore(document.createTextNode(text.substr(0,match.index)+match[1]),node);parent.insertBefore(dom.parseHTML(youtubeEmbedCode(match[2])),node);node.nodeValue=match[3]+text.substr(match.index+match[0].length)}}else{if(!dom.is(node,'code')){convertYoutubeLinks(node)}}
node=node.nextSibling}};sceditor.plugins.autoyoutube=function(){this.signalPasteRaw=function(data){if(dom.closest(this.currentNode(),'code')){return}
if(data.text){var html=document.createElement('div');html.textContent=data.text;convertYoutubeLinks(html);data.html=html.innerHTML}}}})(document,sceditor);!function(e){"use strict";e.plugins.undo=function(){var e,t,r=this,o=0,u=50,a=[],n=[],c=!1,s=function(r){c=!0,t=r.value,e.sourceMode(r.sourceMode),e.val(r.value,!1),e.focus(),r.sourceMode?e.sourceEditorCaret(r.caret):e.getRangeHelper().restoreRange(),c=!1},l=function(e,t){var r,o,u,a,n=e.length,c=t.length,s=Math.max(n,c);for(r=0;r<s&&e.charAt(r)===t.charAt(r);r++);for(u=n<c?c-n:0,a=c<n?n-c:0,o=s-1;o>=0&&e.charAt(o-u)===t.charAt(o-a);o--);return o-r+1};r.init=function(){u=(e=this).undoLimit||u,e.addShortcut("ctrl+z",r.undo),e.addShortcut("ctrl+shift+z",r.redo),e.addShortcut("ctrl+y",r.redo)},r.undo=function(){var t=n.pop(),r=e.val(null,!1);return t&&!a.length&&r===t.value&&(t=n.pop()),t&&(a.length||a.push({caret:e.sourceEditorCaret(),sourceMode:e.sourceMode(),value:r}),a.push(t),s(t)),!1},r.redo=function(){var e=a.pop();return n.length||(n.push(e),e=a.pop()),e&&(n.push(e),s(e)),!1},r.signalReady=function(){var r=e.val(null,!1);t=r,n.push({caret:this.sourceEditorCaret(),sourceMode:this.sourceMode(),value:r})},r.signalValuechangedEvent=function(r){var s=r.detail.rawValue;u>0&&n.length>u&&n.shift(),!c&&t&&t!==s&&(a.length=0,(o+=l(t,s))<20||o<50&&!/\s$/g.test(r.rawValue)||(n.push({caret:e.sourceEditorCaret(),sourceMode:e.sourceMode(),value:s}),o=0,t=s))}}}(sceditor);function smf_DraftAutoSave(oOptions)
{this.opt=oOptions;this.bInDraftMode=!1;this.sCurDraftId='';this.oCurDraftDiv=null;this.interval_id=null;this.oDraftHandle=window;this.sLastSaved='';this.bPM=this.opt.bPM?!0:!1;this.sCheckDraft='';setTimeout('addLoadEvent('+this.opt.sSelf+'.init())',4000)}
smf_DraftAutoSave.prototype.init=function()
{if(this.opt.iFreq>0)
{var oIframe=document.getElementsByTagName('iframe')[0];var oIframeWindow=oIframe.contentWindow||oIframe.contentDocument;this.interval_id=window.setInterval(this.opt.sSelf+'.draft'+(this.bPM?'PM':'')+'Save();',this.opt.iFreq);var instanceRef=this;this.oDraftHandle.onblur=function(oEvent){return instanceRef.draftBlur(oEvent,!0)};this.oDraftHandle.onfocus=function(oEvent){return instanceRef.draftFocus(oEvent,!0)};if(oIframeWindow.document)
{var oIframeDoc=oIframeWindow.document;oIframeDoc.body.onblur=function(oEvent){return parent.oDraftAutoSave.draftBlur(oEvent,!1)};oIframeDoc.body.onfocus=function(oEvent){return parent.oDraftAutoSave.draftFocus(oEvent,!1)}}}}
smf_DraftAutoSave.prototype.draftBlur=function(oEvent,source)
{var e=$('#'+this.opt.sSceditorID).get(0);if(sceditor.instance(e).inSourceMode()==source)
{if(this.bPM)
this.draftPMSave();else this.draftSave();if(this.interval_id!="")
window.clearInterval(this.interval_id);this.interval_id=""}
return}
...
{var e=$('#'+this.opt.sSceditorID).get(0);var sPostdata=sceditor.instance(e).getText(!0);var sPosticon=(typeof document.forms.postmodify.icon==='undefined'?'xx':document.forms.postmodify.icon.value);var sPostsubj=(typeof document.forms.postmodify.subject==='undefined'?'':document.forms.postmodify.subject.value);if(isEmptyText(sPostdata)||smf_formSubmitted||this.sCheckDraft==sPostdata)
return!1;if(this.bInDraftMode)
this.draftCancel();document.getElementById('throbber').style.display='';this.bInDraftMode=!0;var aSections=['topic='+parseInt(document.forms.postmodify.elements.topic.value),'id_draft='+(('id_draft' in document.forms.postmodify.elements)?parseInt(document.forms.postmodify.elements.id_draft.value):0),'subject='+escape(sPostsubj.php_to8bit()).replace(/\+/g,"%2B"),'message='+escape(sPostdata.php_to8bit()).replace(/\+/g,"%2B"),'icon='+escape(sPosticon.php_to8bit()).replace(/\+/g,"%2B"),'save_draft=true',smf_session_var+'='+smf_session_id,];if(this.opt.sType=='post')
{if(document.getElementById('check_lock')&&document.getElementById('check_lock').checked)
aSections[aSections.length]='lock=1';if(document.getElementById('check_sticky')&&document.getElementById('check_sticky').checked)
aSections[aSections.length]='sticky=1'}
var e=$('#'+this.opt.sSceditorID).get(0);aSections[aSections.length]='message_mode='+sceditor.instance(e).inSourceMode();sendXMLDocument.call(this,smf_prepareScriptUrl(smf_scripturl)+"action=post2;board="+this.opt.iBoard+";xml",aSections.join("&"),this.onDraftDone);this.sCheckDraft=sPostdata}
smf_DraftAutoSave.prototype.draftPMSave=function()
{var e=$('#'+this.opt.sSceditorID).get(0);var sPostdata=sceditor.instance(e).getText();if(isEmptyText(sPostdata)||smf_formSubmitted||this.sCheckDraft==sPostdata)
return!1;if(this.bInDraftMode)
this.draftCancel();document.getElementById('throbber').style.display='';this.bInDraftMode=!0;var aTo=this.draftGetRecipient('recipient_to[]');var aBcc=this.draftGetRecipient('recipient_bcc[]');var aSections=['replied_to='+parseInt(document.forms.postmodify.elements.replied_to.value),'id_pm_draft='+(('id_pm_draft' in document.forms.postmodify.elements)?parseInt(document.forms.postmodify.elements.id_pm_draft.value):0),'subject='+escape(document.forms.postmodify.subject.value.php_to8bit()).replace(/\+/g,"%2B"),'message='+escape(sPostdata.php_to8bit()).replace(/\+/g,"%2B"),'recipient_to='+aTo,'recipient_bcc='+aBcc,'save_draft=true',smf_session_var+'='+smf_session_id,];if(this.opt.sType=='post')
aSections[aSections.length]='message_mode='+parseInt(document.forms.postmodify.elements.message_mode.value);sendXMLDocument.call(this,smf_prepareScriptUrl(smf_scripturl)+"action=pm;sa=send2;xml",aSections.join("&"),this.onDraftDone);this.sCheckDraft=sPostdata}
smf_DraftAutoSave.prototype.onDraftDone=function(XMLDoc)
{if(!XMLDoc||!XMLDoc.getElementsByTagName('draft'))
return this.draftCancel();this.sCurDraftId=XMLDoc.getElementsByTagName('draft')[0].getAttribute('id');this.sLastSaved=XMLDoc.getElementsByTagName('draft')[0].childNodes[0].nodeValue;document.getElementById(this.opt.sLastID).value=this.sCurDraftId;oCurDraftDiv=document.getElementById(this.opt.sLastNote);setInnerHTML(oCurDraftDiv,this.sLastSaved);if(this.opt.sType=='post')
document.getElementById('draft_section').style.display='none';this.bInDraftMode=!1;document.getElementById('throbber').style.display='none'}
smf_DraftAutoSave.prototype.draftGetRecipient=function(sField)
{var oRecipient=document.forms.postmodify.elements[sField];var aRecipient=[]
if(typeof(oRecipient)!='undefined')
{if('value' in oRecipient)
aRecipient.push(parseInt(oRecipient.value));else{for(var i=0,n=oRecipient.length;i<n;i++)
aRecipient.push(parseInt(oRecipient[i].value));}}
return aRecipient}
smf_DraftAutoSave.prototype.draftCancel=function()
{this.bInDraftMode=!1;document.getElementById('throbber').style.display='none'};function QuickModifyTopic(oOptions)
...
return!1;this.modify_topic_hide_edit(subject.childNodes[0].nodeValue);this.set_hidden_topic_areas('');this.bInEditMode=!1;if($.isFunction($.fn.SMFtooltip));$('.preview').SMFtooltip().smf_tooltip_off;return!1}
...
smf_addButton(this.opt.sButtonStrip,this.opt.bUseImageButton,{sId:this.opt.sSelf+'_restore_button',sText:this.opt.sRestoreButtonLabel,sImage:this.opt.sRestoreButtonImage,sUrl:'#',sCustom:' onclick="return '+this.opt.sSelf+'.handleSubmit(\'restore\')"'});if(this.opt.bCanSplit)
smf_addButton(this.opt.sButtonStrip,this.opt.bUseImageButton,{sId:this.opt.sSelf+'_split_button',sText:this.opt.sSplitButtonLabel,sImage:this.opt.sSplitButtonImage,sUrl:'#',sCustom:' onclick="return '+this.opt.sSelf+'.handleSubmit(\'split\')"'});this.bButtonsShown=!0}
...
return!0;if($('.amt:first').is(':visible'))
$('.amt:first').text(unreadAlerts);else if(!$('.amt:first').is(':visible')&&unreadAlerts!=0)
...
else unreadAlerts=$('.amt:first').text()?$('.amt:first').text():0;unreadAlerts=parseInt(unreadAlerts);$.get(smf_scripturl+'?action=profile;area=alerts_popup;counter='+unreadAlerts+';u='+smf_member_id,function(data)
{var alerts=$(data).find('.unread');if(alerts.length==0)
return!0;unreadAlerts+=alerts.length;if(unreadAlerts!==parseInt(unreadAlerts))
return!0;if(typeof localStorage!='undefined')
localStorage.setItem('alertsCounter',unreadAlerts);if($('.amt:first').is(':visible'))
$('.amt:first').text(unreadAlerts);else if(!$('.amt:first').is(':visible')&&unreadAlerts!=0)
$('#alerts_menu_top').append('<span class="amt">'+unreadAlerts+'</span>');$.each(alerts,function(index,item)
{var notification=notify.createNotification(new_alert_title,{body:$(item).find('div.details:first > span').text(),icon:smf_images_url+'/blank.png'});notification.click(function()
{window.focus();if(!$('#alerts_menu').is(':visible'))
$('#alerts_menu_top').click()});if(alert_timeout>0)
setTimeout(function()
{notification.close()},alert_timeout)})});setTimeout(updateAlerts,pingTime)}
$(function()
...
localStorage.setItem('alertsCounter',parseInt($('.amt:first').text()?$('.amt:first').text():0))}});(function(win){var PERMISSION_DEFAULT="default",PERMISSION_GRANTED="granted",PERMISSION_DENIED="denied",PERMISSION=[PERMISSION_GRANTED,PERMISSION_DEFAULT,PERMISSION_DENIED],defaultSetting={pageVisibility:!1,autoClose:0},empty={},emptyString="",isSupported=(function(){var isSupported=!1;try{isSupported=!!(win.Notification||win.webkitNotifications||navigator.mozNotification||(win.external&&win.external.msIsSiteMode()!==undefined))}catch(e){}
