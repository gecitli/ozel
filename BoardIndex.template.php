		...
		echo '
			...
			foreach ($category['boards'] as $board)
			{
				...
				echo '
					...
					<div class="lastpost ',!empty($board['last_post']['id']) ? 'lpr_border' : 'hidden', '">'.$board['last_post']['member']['avatar']['image'].'', function_exists('template_bi_' . $board['type'] . '_lastpost') ? call_user_func('template_bi_' . $board['type'] . '_lastpost', $board) : template_bi_board_lastpost($board), '
					</div>
					<div class="lastpost_mobile ',!empty($board['last_post']['id']) ? 'lpr_border' : 'h', '"><i class="fas fa-ellipsis-v"></i>', function_exists('template_bi_' . $board['type'] . '_lastpost') ? call_user_func('template_bi_' . $board['type'] . '_lastpost', $board) : template_bi_board_lastpost($board), '
					</div>';
				...
				<table>
					...
					echo'
						<tr class="windowbg', $alter ? 2 : '', '">
							<td width="2%" class="icon2 avatared"><img class="avatar" src="', $ava, '" /></td>
							...
							<td align="right" class="phoneNone">', $post['board']['link'], '<br>', $post['time'], '</td>
						</tr>';
						$alter = !$alter;
					}
				echo'
