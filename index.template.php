...
function template_body_above()
{
			...
			if (!empty($modSettings['userLanguage']) && !empty($context['languages']) && count($context['languages']) > 1)
	{
		echo '
			<form id="languages_form" method="get" class="floatright">
				<select class="custom-select" id="inputGroupSelect01 language_select" name="language" onchange="this.form.submit()">';
	...
	echo '
				...
				<div id="accordion" role="tablist" aria-multiselectable="true">
					<a class="wp" data-toggle="collapse" data-parent="#accordion" href="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
						...
						<div class="search-input-wrapper">
							<div class="search-input">
								<form action="', $scripturl, '?action=search2" method="post" accept-charset="', $context['character_set'], '">
									<input name="search" type="search" id="mobile_quicksearch" class="form-control input-sm inline-block" placeholder="Search...">
...
function template_button_strip($button_strip, $direction = '', $strip_options = array())
{
	...
	foreach ($button_strip as $key => $value)
	{
		...
		if (!isset($value['test']) || !empty($context[$value['test']]))
		{
			...
			$button = '
				<a class="button button_strip_' . $key . (!empty($value['active']) ? ' active' : '') . (isset($value['class']) ? ' ' . $value['class'] : '') . '" ' . (!empty($value['url']) ? 'href="' . $value['url'] . '"' : '') . ' ' . (isset($value['custom']) ? ' ' . $value['custom'] : '') . '><i class="fa fa-'.$value['text'].' fa-fw"></i> <span class="hidden-xs">' . $txt[$value['text']] . '</span></a>';
	...
	echo '
		...
		echo '
		<div class="buttonlist-mobile', !empty($direction) ? ' float' . $direction : '', '"', (empty($buttons) ? ' style="display: none;"' : ''), (!empty($strip_options['id']) ? ' id="' . $strip_options['id'] . '"' : ''), '>
